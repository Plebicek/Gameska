...
public class UFO {
    ...
    public UFO(int width, int height, int x, int y, int MAX_POSITION, int MIN_POSITION) {
        ...
        UFOImage = imageLoader.load("UFO.png");
        //movement
    ...
    public void isCollisionWithPlayer(Player player, Game game){
        if(game.getDisplay().getGameState()==STATE.LVL2){
            if (this.getPosition().getX() < player.getPosition().getX()+63 && this.getPosition().getX()+63 > player.getPosition().getX() && this.getPosition().getY() < player.getPosition().getY()+39 && this.getPosition().getY()+35 > player.getPosition().getY()) {
                    game.getDisplay().setGameState(STATE.GAMEOVER);
                }
            }
    }
